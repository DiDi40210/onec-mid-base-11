...
Асинх Процедура СогласованнаяСкидкаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Товары) ИЛИ ЗначениеЗаполнено(Объект.Услуги) Тогда
		...
		Вопрос = ВопросАсинх ("Пересчитать табличную часть?", РежимДиалогаВопрос.ДаНет);
...
Процедура РассчитатьСуммуСтроки(ТекущиеДанные)
	...
	ИтоговаяСкидка = ТекущиеДанные.Скидка + Объект.УК_СогласованнаяСкидка;  
	 
	 Если ИтоговаяСкидка > 100 Тогда
		Сообщить ("Скидка не может быть больше 100%");
	КонецЕсли;
	
	 ИтоговаяСкидка = Мин(100, ИтоговаяСкидка);  
